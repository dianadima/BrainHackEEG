...
% Note: code available with BrainStorm toolbox
...
%% edit - moved this below to ensure SD from training data is used
...
for p = 1:numpermutation
    ...
    sd = std(traindata,[],3,'omitnan');
    traindata = bsxfun(@rdivide,traindata,sd); %important step to equaly weight all variables
    testdata = bsxfun(@rdivide,testdata,sd);
    if conditiongen
        testdata2 = bsxfun(@rdivide,testdata2,sd); %important step to equally weight all variables
    end
